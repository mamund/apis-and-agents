...
async function main() {
  program
    ...
    .option('--use-state-ref [file]', 'Load sharedStateURL from a reference file (default: .job-runner-ref.json)')
    .option('--save-state-ref [file]', 'Save sharedStateURL to a reference file (default: .job-runner-ref.json)')
    .parse();
  ...
  const defaultStateRefFile = '.job-runner-ref.json';
  const useStateRefFile = options.useStateRef !== undefined ? options.useStateRef || defaultStateRefFile : null;
  const saveStateRefFile = options.saveStateRef !== undefined ? options.saveStateRef || defaultStateRefFile : null;

  let sharedStateURL = null;
  ...
  if (options.sharedStateUrl) {
    sharedStateURL = options.sharedStateUrl;
    logVerbose(verbose, 'Using sharedStateURL from --shared-state-url:', sharedStateURL);
  } else if (useStateRefFile) {
    const ref = JSON.parse(fs.readFileSync(useStateRefFile, 'utf-8'));
    sharedStateURL = ref.sharedStateURL;
    logVerbose(verbose, `Using sharedStateURL from --use-state-ref (${useStateRefFile}):`, sharedStateURL);
  } else if (stateFile) {
    const state = JSON.parse(fs.readFileSync(stateFile, 'utf-8'));
    stateId = state.id || uuidv4();
    state.id = stateId;

    sharedStateURL = `${stateURL}/state/${stateId}`;
    stateWasCreated = true;

    logVerbose(verbose, 'Prepared shared state:', state);
    logVerbose(verbose, 'sharedStateURL to be used:', sharedStateURL);
  ...
  } else if (config.sharedStateURL) {
    sharedStateURL = config.sharedStateURL;
    logVerbose(verbose, 'Using sharedStateURL from config:', sharedStateURL);
  }
  ...
  if (!dryRun) {
    ...
    if (saveStateRefFile && sharedStateURL) {
      fs.writeFileSync(saveStateRefFile, JSON.stringify({ sharedStateURL }, null, 2), 'utf-8');
      logVerbose(verbose, `Saved sharedStateURL to ${saveStateRefFile}`);
    }
  } else {
  ...
  if (!keep && stateWasCreated && stateId && !dryRun) {
